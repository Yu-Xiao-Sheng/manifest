...
describe('ProxyController', () => {
  ...
  beforeEach(() => {
    ...
    providerClient = {
      ...
      createAnthropicStreamTransformer: jest.fn(() => jest.fn()),
      convertChatGptResponse: jest.fn(),
      convertChatGptStreamChunk: jest.fn(),
      createChatGptStreamTransformer: jest.fn(() => jest.fn()),
    };
    mockMessageManager = {
  ...
  it('should force ChatGPT responses through the streaming transformer', async () => {
  ...
  describe('first proxy request tracking', () => {
    ...
    it('should not fire event on provider error for already-seen user', async () => {
      ...
      proxyService.proxyRequest.mockResolvedValue({
        forward: { response: errorResp, isGoogle: false, isAnthropic: false, isChatGpt: false },
        meta: {
          tier: 'complex',
          model: 'gpt-4o',
          provider: 'OpenAI',
