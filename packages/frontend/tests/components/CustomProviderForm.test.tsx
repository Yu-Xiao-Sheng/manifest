...
describe('CustomProviderForm', () => {
  ...
  it('calls createCustomProvider and onCreated on submit', async () => {
    ...
    await waitFor(() => {
      expect(mockCreateCustomProvider).toHaveBeenCalledWith('test-agent', {
        ...
        responseAPIConfig: undefined,
        models: [{ model_name: 'llama-3.1-70b', supports_response_api: false }],
      });
  ...
  it('filters out empty model names on submit', async () => {
    ...
    await waitFor(() => {
      expect(mockCreateCustomProvider).toHaveBeenCalledWith('test-agent', {
        ...
        enableResponseAPI: false,
        responseAPIConfig: undefined,
        models: [{ model_name: 'model-a', supports_response_api: false }],
      });
    });
  });
  ...
  it('resets response API fields when switching to a different custom provider', async () => {
    const providerA = {
      id: 'cp-1',
      name: 'Provider A',
      base_url: 'https://provider-a.example/v1',
      has_api_key: true,
      models: [{ model_name: 'gpt-a', supports_response_api: false }],
      created_at: '2026-03-04T00:00:00Z',
    };
    const providerB = {
      id: 'cp-2',
      ...
      has_api_key: true,
      models: [{ model_name: 'gpt-b', supports_response_api: true }],
      enable_response_api: true,
      response_api_config: {
        audio: { input: true, output: false },
        screen: { capture: true, analysis: false },
        streaming: true,
      },
      created_at: '2026-03-04T00:00:00Z',
    };

    let setProvider!: (provider: typeof providerA | typeof providerB) => void;

    render(() => {
      const [provider, setProviderSignal] = createSignal<typeof providerA | typeof providerB>(
        providerA,
      );
      setProvider = setProviderSignal;
      return (
        <CustomProviderForm
    ...
    expect(screen.getByDisplayValue('Provider A')).toBeDefined();
    expect((screen.getByLabelText('Enable OpenAI Response API') as HTMLInputElement).checked).toBe(
      false,
    );
    ...
    await waitFor(() => {
      expect(screen.getByDisplayValue('Provider B')).toBeDefined();
      expect(
        (screen.getByLabelText('Enable OpenAI Response API') as HTMLInputElement).checked,
      ).toBe(true);
    });

    expect(
      (screen.getByLabelText('Input (audio input from user)') as HTMLInputElement).checked,
    ).toBe(true);
    expect((screen.getByLabelText('Capture (take screenshots)') as HTMLInputElement).checked).toBe(
      true,
    );
    expect((screen.getByLabelText('Enable streaming responses') as HTMLInputElement).checked).toBe(
      true,
    );
  });
});
...
describe('CustomProviderForm — edit mode', () => {
  ...
  it('renders with pre-populated fields', () => {
    ...
    expect((screen.getByPlaceholderText('e.g. Groq, vLLM, Azure') as HTMLInputElement).value).toBe(
      'Groq',
    );
    expect(
